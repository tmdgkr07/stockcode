...
// console.log(Object.keys(gptlist))
// try{
// if(!gptlist[msg.room].sender.includes(msg.sender.name)){
//   if(Object.keys(gptlist).includes(msg.room))
//   {console.log(msg.room)}
//  var test4 = `{"${msg.room}":{"sender":"${msg.sender.name}","count":3,"maxcount":10}}`
//  fs.writeFileSync('./INFO/gpt.json',JSON.stringify(test4))
//   }
...
//     console.log(err);
//     var test4 = `{"${msg.room}":{"sender":"${msg.sender.name}","count":3,"maxcount":10}}`
//     fs.writeFileSync('./INFO/gpt.json',test4)
//   }
...
if (cmd === 'ping') {
    ...
    */
    ...
    msg.reply(`${Date.now() - timestamp}ms`,'이승학');
}
   ...
   await delay(delay_time)
    ...
    if (messageQueue.length > 0){
        ...
        if(isNaN(stock_data.stock_chat[0])){
          ...
          let stock_name23 = Object.keys(stockdata);
          let stock_value = Object.values(stockdata);
          let stockdata = stock_name23.concat(stock_value);
