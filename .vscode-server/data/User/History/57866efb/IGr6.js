    ...
    try{
     request.get({
        ...
        if(err){
            ...
            var data3 = data.split("0(")
           // console.log(data[1])
            var data2= data3[1].slice(0,-2)
            data2 = data2.replace('result', '"result"')
            data2 = data2.replace('day', '"day"')
            data2.result.day = data2.result.day.replace('title', '"title"')
            data2.result.day = data2.result.day.replace('date', '"date"')
            data2.result.day = data2.result.day.replace('content', '"content"')
            data2 = data2.replace('tomorrow', '"tomorrow"')
            data2 = data2.replace('month', '"month"')
            data2 = data2.replace('resultMSG', '"resultMSG"')
            const obj_data = JSON.parse(data2)
